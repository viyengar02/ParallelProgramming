...
int pso_get_best_fitness(swarm_t *swarm, int num_threads)
{
    int i, g, tid;
    float best_fitness = INFINITY;
	int g_[num_threads];
	float bf_[num_threads];

    particle_t *particle; 
    g = -1;
	int swarm_size = swarm->num_particles;

	#pragma omp parallel private(i, tid, particle) firstprivate(swarm_size, num_threads, best_fitness) shared(swarm, g, g_, bf_) num_threads(num_threads)
    {
		//check multithreaded
		if(num_threads > 1){
	...
	for(i = 0; i < num_threads; i++){
		if (bf_[i] < best_fitness){
			g = g_[i];
			best_fitness = bf_[i]; //updates best particle index
		}
	}
...
/* Initialize PSO */
//paralellized 
swarm_t *pso_init(char *function, int dim, int swarm_size, float xmin, float xmax, int num_threads){
    int i, j, g, status;
    float fitness;
    swarm_t *swarm;
    particle_t *particle;
	...
	if( !(strcmp(function, "booth") == 0 || strcmp(function, "rastrigin") == 0 || strcmp(function, "holder_table") == 0 || strcmp(function, "eggholder") == 0 || strcmp(function, "schwefel") == 0)){
		fprintf(stderr, "Could not evaluate fitness. Unknown function provided.\n");
		pso_free(swarm);
		return NULL;
	} 

	#pragma omp parallel for private(i, j, status, particle, fitness) firstprivate(dim, swarm_size, xmin, xmax)  shared(swarm, g) num_threads(num_threads)
	for (i = 0; i < swarm_size; i++) {
		particle = &swarm->particle[i];
		particle->dim = dim; 
	...
	#pragma omp parallel for private(i) firstprivate(g) shared(swarm) num_threads(num_threads)
	for (i = 0; i < swarm_size; i++){
		particle = &swarm->particle[i];
		particle->g = g;
	}
